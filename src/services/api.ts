...
export class ApiService {
  ...
  // 恢复仿真
  static async resumeSimulation(simulationId: string): Promise<void> {
    await apiClient.post(`/simulation/${simulationId}/resume`)
  }
  ...
  static async getSimulationResults(simulationId: string): Promise<{
    ...
    paraview_file?: string
    paraview_web_url?: string
    paraview_web_ws_url?: string
  }> {
  ...
  // 获取 ParaViewWeb 信息
  static async getParaviewWeb(simulationId: string): Promise<{
    status: string
    url?: string
    port?: string
    ws_url?: string
    ...
    const response = await apiClient.get(`/simulation/${simulationId}/paraview-web`)
    return response.data
  }

  // 启动 ParaView（可选指定结果文件）
  static async launchParaview(caseFile?: string): Promise<void> {
    await apiClient.post('/tools/paraview/launch', { case_file: caseFile })
  }

  // 打开本地路径（结果目录）
  static async openPath(path: string): Promise<void> {
    await apiClient.post('/tools/open-path', { path })
  }
    ...
    try {
      ...
      this.ws.onclose = (event) => {
        ...
        const shouldReconnect = !(
          (event.code === 1000 && event.reason === '正常关闭') ||
          event.reason === '服务器关闭'
        )
