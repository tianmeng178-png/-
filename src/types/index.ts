...
export interface MicrochannelParameters {
  ...
  // 求解参数
  mesh_resolution: number;     // mock: cell count, openfoam: cell size (m)
  convergence_criteria: number; // 收敛标准
  
  // 仿真模式
  simulation_mode: 'mock' | 'openfoam'; // 仿真模式：模拟模式或真实OpenFOAM
  use_gpu_acceleration?: boolean; // 是否使用GPU加速（如果可用）
}
